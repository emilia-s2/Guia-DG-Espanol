		...
		if ([213, 214].includes(skillid)) { // Ice inside
			dispatch.setTimeout(() => {
				if (debuff != null) {
					handlers.text({
						sub_type: "message",
						message_ES: (`Hielo (${qbacting_messages[qbacting].message}) | ${mech_messages[debuff % 2 + 2].message} | ${mech_messages[(qbacting + debuff + 1) % 2].message}`),
		...
		if ([212, 215].includes(skillid)) { // Fire inside
			dispatch.setTimeout(() => {
				if (debuff != null) {
					handlers.text({
						sub_type: "message",
						message_ES: (`Fuego (${qbacting_messages[qbacting].message}) | ${mech_messages[debuff % 2 + 2].message} | ${mech_messages[(qbacting + debuff) % 2].message}`),
...
//		"157-0": [{ type: "text", sub_type: "message", message_ES: "Change", message_RU: "Смена" }],
		...
		"114-0": [{ type: "text", sub_type: "message", message_ES: "Frente | Fuego", message_RU: "Огонь впереди" }],
...
//		"118-1": [{ type: "text", sub_type: "message", message_ES: "Dodge", message_RU: "Эвейд!" }],
		...
		// AOE огонь (большой)
		"105-0": [{ type: "text", sub_type: "message", message_ES: "AOE de FUEGO (GRANDE)", message_RU: "Огненные бомбы" },
		...
		// AOE лед (малый)
		"154-0": [{ type: "text", sub_type: "message", message_ES: "Magia de HIELO (Pequeno)", message_RU: "Ледяной шторм" }, { type: "spawn", func: "circle", args: [false, 553, 0, 80, 8, 520, 0, 6500] }],
		// AOE огонь (малый)
...
//		"206-2": [{ type: "spawn", func: "circle", args: [false, 553, 0, 0, 15, 350, 0, 3000] }],
		...
		"am-3126-1000-31260068": [
			...
			{ type: "text", sub_type: "message", delay: 145000, message_ES: "[Debuff] 2.5 minutos pasado", message_RU: "[Дебафф] Прошло 2.5 минуты (стаки удалены)" }
		]
	};
